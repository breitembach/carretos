...
@Injectable()
export class DetailUserBackendService {
    ...
    public getUserById = (userId: UUID): Observable<any> => {
        const endpoint = `${this._carretosApi}users/${userId}`;
        return this.http.get(endpoint)
            .map(res => {
                return res.json();
            })
            .do(data => console.log('[getUserById] server data: ', data))
            .catch(err => this._serverError(err, 'getUserById'));
    }
